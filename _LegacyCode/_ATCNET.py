...
and those 2 GitHub repositories: https://github.com/Altaheri/EEG-ATCNet 
https://github.com/doc-ycm/EEG-ATCNet-pytorch
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        Q = Q.view(batch_size, seq_len, self.num_heads, self.hidden_size).permute(
            0, 2, 1, 3
        )  # (batch_size, num_heads, seq_len, hidden_size)
        K = K.view(batch_size, seq_len, self.num_heads, self.hidden_size).permute(
            0, 2, 1, 3
        )  # (batch_size, num_heads, seq_len, hidden_size)
        V = V.view(batch_size, seq_len, self.num_heads, self.hidden_size).permute(
            0, 2, 1, 3
        )  # (batch_size, num_heads, seq_len, hidden_size)
        ...
        attn_output = (
            attn_output.permute(0, 2, 1, 3).contiguous().view(batch_size, seq_len, -1)
...
class conv_block(nn.Module):
    def __init__(
        ...
        self.depthwise = nn.Conv2d(
            16, 16, (22, 1), stride=1, padding=0, dilation=1, groups=16, bias=False
        )
...
def train():
    ...
    for i in range(n_sub):
        ...
        test_loader = torch.utils.data.DataLoader(
            dataset=test_dataset, batch_size=300, shuffle=False
        )
        ...
        for epoch in range(n_epochs):
            ...
            for k, batch in enumerate(train_loader):
                ...
                acc = (
                    (pred.argmax(dim=-1) == label.to(device).argmax(dim=-1))
                    .float()
                    .mean()
                )
            ...
            for k, batch in enumerate(test_loader):
                ...
                acc = (
                    (logits.argmax(dim=-1) == label.to(device).argmax(dim=-1))
                    .float()
                    .mean()
                )
