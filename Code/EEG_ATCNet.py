...
class ConvBlock(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernLength=64, poolSize=8, D=2, dropout=0.1
    ):
        super(ConvBlock, self).__init__()
        F1 = out_channels
        F2 = F1 * D
        self.conv1 = nn.Conv2d(
            in_channels, F1, kernel_size=(kernLength, 1), padding="same"
        )  # Use 'same' for automatic padding
        ...
        self.conv2 = nn.Conv2d(
            F2, F2, kernel_size=(16, 1), padding="same"
        )  # Use 'same' for automatic padding
...
class TCNBlock(nn.Module):
    def __init__(self, input_dimension, depth, kernel_size, filters, dropout):
        ...
        self.layers.append(
            nn.Conv1d(input_dimension, filters, kernel_size=1, padding=0)
        )
...
class ATCNet(nn.Module):
    def __init__(
        ...
        self.tcn_block = TCNBlock(
            eegn_F1, tcn_filters, tcn_kernelSize, tcn_dropout, tcn_activation
        )
