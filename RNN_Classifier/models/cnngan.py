...
class CNNCGANGenerator(Generator):
    def __init__(
        ...
        **kwargs
        ...
        super(CNNCGANGenerator, self).__init__(self.encoding_dims, self.label_type)
        self.label_embeddings = nn.Embedding(
            self.num_classes, self.label_embedding_size
        )
    ...
    def sampler(self, sample_size, device="cpu"):
        return [
            ...
            torch.multinomial(self.prob_classes, sample_size, replacement=True).to(
                device
            ),
        ]
...
class CNNCGANDiscriminator(Generator):
    def __init__(self, input_size, channel_last=True, hidden_size=50, **kwargs):
        # Defaults
        ...
        super(CNNCGANDiscriminator, self).__init__(self.input_size, self.label_type)
