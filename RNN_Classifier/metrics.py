...
def tstr(X_synth, y_synth, X_real, y_real, epochs=3_000, batch_size=None):
    ...
    sampler_train = BinaryBalancedSampler(
        X_train, y_train, tile=True, batch_size=batch_size
    )
    ...
    trainer = BinaryClassificationTrainer(
        model, optimizer, sampler_train, sampler_test, metrics_prepend="tstr_"
    )
...
def classify(X, y, epochs=3_000, batch_size=None):
    ...
    (X_train, y_train, X_test, y_test) = train_test_split(
        X, y, 0.3
    )  # Warning! modified original file
    (X_train, y_train, X_test, y_test) = numpy_to_cuda(X_train, y_train, X_test, y_test)
    y_test, y_train = y_test.float(), y_train.float()
    ...
    optimizer = optim.Adam(model.parameters(), lr=0.001)
    trainer = BinaryClassificationTrainer(model, optimizer, sampler_train, sampler_test)
